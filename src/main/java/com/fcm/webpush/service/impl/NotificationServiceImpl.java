...
public class NotificationServiceImpl implements NotificationService {
	...
	public SubscriptionResponseDto registerOrRefreshSubscription(final SubscriptionRequestDto request, final String userId) {
		var targetGuestId = request.getGuestId();
		...
		if (targetGuestId == null || targetGuestId.isBlank()) {
			...
			final var guestExistsInDb = subscriptionRepository.existsByGuestId(targetGuestId);
			if (guestExistsInDb)
		...
		final var subscription = subscriptionRepository.findByFcmToken(request.getFcmToken())
				...
				.orElseGet(() -> {
					final var guestSubs = subscriptionRepository.findAllByGuestId(finalGuestId);
					if (!guestSubs.isEmpty()) {
						final var existingGuestSub = guestSubs.get(0);
						final var oldToken = existingGuestSub.getFcmToken();
	...
	public void detachUserFromSubscription(final String guestId, final String fcmToken) {
		...
		if (fcmToken != null && !fcmToken.isBlank())
			subscriptionRepository.findByFcmToken(fcmToken).ifPresent(sub -> {
				final var previousUserId = sub.getUserId();
				sub.setUserId(null);
			...
			final var subs = subscriptionRepository.findAllByGuestId(guestId);
	...
	public SendNotificationResponseDto sendNotification(final SendNotificationRequestDto request) {
		...
		final var hasUsers 		= request.getUserIds() != null && !request.getUserIds().isEmpty();
		final var hasGuests 	= request.getGuestIds() != null && !request.getGuestIds().isEmpty();
		...
		final var distinctUserIds = hasUsers
				? request.getUserIds().stream().distinct().toList()
						: List.<Long>of();

		final var distinctGuestIds = hasGuests
				? request.getGuestIds().stream().distinct().toList()
						: List.<String>of();

		if (!distinctUserIds.isEmpty()) {
			final var foundUsers = userRepository.findAllById(distinctUserIds);
			if (foundUsers.size() < distinctUserIds.size()) {
				final var foundIds 		= foundUsers.stream().map(User::getId).collect(Collectors.toSet());
				final var missingIds	 	= distinctUserIds.stream().filter(id -> !foundIds.contains(id)).toList();
		...
		if (!distinctGuestIds.isEmpty()) {
			final var guestSubs = subscriptionRepository.findByGuestIdIn(distinctGuestIds);
			final var foundGuestIds 	= guestSubs.stream().map(NotificationSubscription::getGuestId).collect(Collectors.toSet());
			final var missingGuestIds 	= distinctGuestIds.stream().filter(id -> !foundGuestIds.contains(id)).toList();
		...
		for (final var gId : distinctGuestIds)
			logsToSave.add(NotificationLog.builder()
					.guestId(gId)
					.templateId(template.getId())
					.title(template.getTitle())
		...
		if (!distinctGuestIds.isEmpty()) {
			final var guestSubs = subscriptionRepository.findByGuestIdInAndIsActiveTrue(distinctGuestIds);
		...
		final var tokenList = new ArrayList<>(tokensToNotify);
		var sentCount = 0;
		var failedCount = 0;
		final var invalidTokens = new ArrayList<String>();

		for (var i = 0; i < tokenList.size(); i += FCM_BATCH_SIZE) {
			...
			try {
				final var response = firebaseMessaging.sendEachForMulticast(message);
				sentCount 		+= response.getSuccessCount();
				failedCount 	+= response.getFailureCount();

				final var responses = response.getResponses();
				for (var j = 0; j < responses.size(); j++) {
					final var sendResponse = responses.get(j);
					if (!sendResponse.isSuccessful()) {
						final var exception = sendResponse.getException();
	...
	private boolean isInvalidTokenError(final FirebaseMessagingException exception) {
		final var code = exception.getMessagingErrorCode();
	...
	private void deactivateInvalidTokens(final List<String> invalidTokens) {
		if (invalidTokens == null || invalidTokens.isEmpty())
			return;
		final var count = subscriptionRepository.deactivateTokensInBulk(invalidTokens, Instant.now());
		log.info("EVENT=TOKENS_DEACTIVATED Deactivated {} stale/invalid FCM tokens in bulk", count);
	}
